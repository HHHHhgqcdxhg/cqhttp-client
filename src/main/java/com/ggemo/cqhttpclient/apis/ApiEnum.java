...
GET_CREDENTIALS("get_credentials", " 获取 QQ 相关接口凭证", "即上面两个接口的合并。"),
GET_RECORD("get_record", " 获取语音", "其实并不是真的获取语音，而是转换语音到指定的格式，然后返回语音文件名（`data\record` 目录下）。**注意，要使用此接口，需要安装 酷Q 的 [语音组件](https://cqp.cc/t/21132)。**"),
...
SET_RESTART_PLUGIN("set_restart_plugin", " 重启 HTTP API 插件", "由于重启插件同时需要重启 API 服务，这意味着当前的 API 请求会被中断，因此需在异步地重启插件，接口返回的 `status` 是 `async`。"),
CLEAN_DATA_DIR("clean_data_dir", " 清理数据目录", "用于清理积攒了太多旧文件的数据目录，如 `image`。"),
CLEAN_PLUGIN_LOG("clean_plugin_log", " 清理插件日志", "用于清空插件的日志文件。");
